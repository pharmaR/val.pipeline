...
**This file lives only on the `ac-dev` branch.** Never merge `ac-dev`
outward and never `git add` `.github/copilot-instructions.md`,
`.github/agent-plan.md`, or `.github/ISSUE_TEMPLATE/agent-loop.md` on any
other branch. `ac-dev` is a one-way local superset of `main` — see the
branch model below.
...
- **`ac-dev`** — Aaron's **local-only** testing superset branch and the
  ONLY branch that carries agent files (`.github/copilot-instructions.md`,
  `.github/agent-plan.md`, `.github/ISSUE_TEMPLATE/agent-loop.md`).
  Never pushed to `origin`. Never merged outward. After opening a PR
  from `ac-<n>-slug`, merge that branch **into** `ac-dev` so local
  iteration runs the newest code:
  ```
  git checkout ac-dev
  ...
  git fetch origin
  git checkout ac-dev
  git merge --ff-only origin/main   # keeps agent files on top
  ```

**Cross-branch hygiene**: agent files are tracked on `ac-dev` only. On
any `ac-<n>-slug` / `jt-*` / `main` checkout they simply won't exist in
the working tree (git leaves untracked-elsewhere files alone on
checkout, so if they persist locally, that's fine — just don't add
them). **Never `git add` them on a non-`ac-dev` branch.** If a stray
agent file shows up on a fix branch, `git rm --cached` it before
committing.
